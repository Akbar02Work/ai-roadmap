-- Phase 4: Atomic roadmap generation with per-goal serialization.

BEGIN;

CREATE UNIQUE INDEX IF NOT EXISTS roadmaps_goal_version_uq
    ON public.roadmaps(goal_id, version);

CREATE UNIQUE INDEX IF NOT EXISTS roadmaps_goal_active_uq
...
BEGIN
    ...
    RETURN v_new_roadmap_id;
END;
$$;

REVOKE ALL ON FUNCTION public.generate_roadmap_v1(uuid, text, text, jsonb, jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.generate_roadmap_v1(uuid, text, text, jsonb, jsonb) TO authenticated;
